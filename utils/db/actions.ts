  ...
  export async function redeemReward(userId: number, rewardId: number) {
    try {
      ...
      if (rewardId === 0) {
        ...
        if (!userReward || !availableReward[0] || userReward.points < availableReward[0].point) {
          throw new Error("Insufficient points or invalid reward");
        }
  
        const [updatedReward] = await db.update(Rewards)
          .set({ 
            point: sql`${Rewards.point} - ${availableReward[0].point}`,
            updatedAt: new Date(),
          })
          .where(eq(Rewards.userId, userId))
          .returning()
        ...
        // Create a transaction for this redemption
        await createTransaction(userId, 'redeemed', availableReward[0].point, `Redeemed: ${availableReward[0].name}`);
